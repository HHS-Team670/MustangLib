...
public class TimeOfFlightSensor {
  ...
  public TimeOfFlightSensor(I2C.Port port, int address, int threshold) {
    ...
    if (isMultiplexer && !(address <= 7 && address >= 0)) {
      MustangNotifications.reportError("TOF Sensor address out of range. Expected 0-7. Given: %s",
          address);
    }
   ...
   *        the register in the sensor's memory where the data will be written to. It is used to
   *        specify which register the data should be written to.
   * @param data The "data" parameter is an integer value that represents the data to be written to
  ...
  private boolean write(int registerAddress, int data) {
    try {
      ...
      MustangNotifications
          .reportError("Write for sensor %s could not be performed. Check connection", address);
    }
    return true;
  }
  ...
  private int readShortInt(int registerAddress) {
    try {
      ...
      MustangNotifications
          .reportError("Read for sensor %s could not be performed. Check connection", address);
    }
    return ERROR;
  }
   ...
   *        level. It is used to set a threshold value for a specific condition or action in a
   *        program.
   */
