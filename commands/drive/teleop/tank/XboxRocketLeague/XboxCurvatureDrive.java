 ...
 * rather than adding/subtracting linearly from the wheel speeds. The turn output is a sum of the
 * throttle and turn inputs, meaning that the robot will turn faster when it's moving forward at a
 * higher speed. Again, the goal here is more control at low speeds. Note that the turn input is not
 * changed when the robot is turning in place, this just affects the way the robot drives in an "S"
 * shaped path. Cheesy Drive also applies some non-linearity to the joystick input so that there is
 * more control at the low speeds. Larger changes in joystick inputs result in smaller changes in
 * real speed when driving slow, but when the joystick is pushed to a high speed, you jump up to
 * full speed faster. Third, that turn input is affected by a negative inertia accumulator. Most
 * robots have a fair bit of turning inertia, which can make it easy to accidentally overshoot a
 * turn. The negative inertia accumulator acts almost like a reverse integral controller the longer
 * the robot has been turning (fast) for, the slower the robot will turn.
 */
