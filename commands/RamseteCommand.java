 ...
 *
 * <p>
 * The command handles trajectory-following, PID calculations, and feedforwards internally. This is
 * intended to be a more-or-less "complete solution" that can be used by teams without a great deal
 * of controls expertise.
 *
 * <p>
...
public class RamseteCommand extends Command {
   ...
   *
   * <p>
   * Note: The controller will *not* set the outputVolts to zero upon completion of the path - this
  ...
  public RamseteCommand(Trajectory trajectory, Supplier<Pose2d> pose, RamseteController controller,
      SimpleMotorFeedforward leftFeedForward, SimpleMotorFeedforward rightFeedForward,
  ...
  public void initialize() {
    ...
    m_prevSpeeds =
        m_kinematics.toWheelSpeeds(new ChassisSpeeds(initialState.velocityMetersPerSecond, 0,
  ...
  public void execute() {
    ...
    if (m_usePID) {
      double leftFeedforward = m_leftFeedForward.calculate(leftSpeedSetpoint,
          (leftSpeedSetpoint - m_prevSpeeds.leftMetersPerSecond) / dt);

      double rightFeedforward = m_rightFeedForward.calculate(rightSpeedSetpoint,
    ...
    ConsoleLogger.consoleLog(
        "LeftspeedSetPoint: %s, Left Speed: %s, LeftOutput: %s, rightSpeedSetPoint: %s Right Speed: %s rightOutput: %s",
        leftSpeedSetpoint, m_speeds.get().leftMetersPerSecond, leftOutput, rightSpeedSetpoint,
        m_speeds.get().rightMetersPerSecond, rightOutput);
