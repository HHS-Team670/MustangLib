...
public class MustangScheduler {
  ...
  /**
   * To be used only as a convenience feature for quickly scheduling command groups. Expects nothing
   * less than a green health state for the subsystem
   *
    ...
    if (command == null) {
      ...
      if (!(command instanceof MustangCommand)) {
        MustangNotifications.reportError(
            "%s was not properly scheduled. Are you using MustangScheduler?", command.getName());
      }
    }
  }
