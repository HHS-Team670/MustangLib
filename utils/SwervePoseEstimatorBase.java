...
public abstract class SwervePoseEstimatorBase {
    ...
    public void initVision(VisionSubsystemBase vision) {
        ...
        // The commented code is loading a single substation pose to the SmartDashboard. It flips
        // the
        // pose based on the alliance color and sets it as the pose for the "Single Substation"
        // object
        // in the Field2d. It then puts the Field2d object and the formatted pose on the
        // SmartDashboard.
        // TODO move to 2023 // // load single sub to smartdashboard
        // Pose2d singlesub =
        // FieldConstants.allianceFlip(FieldConstants.LoadingZone.IntakePoses[0]);
    ...
    public void update() {
        ...
        if (vision != null) {
            ...
            while (!vision.isMeasurementBufferEmpty()) {
                ...
                poseEstimator.addVisionMeasurement(estimatedPose, estimation.timestampSeconds,
                        m.confidence());
                Logger.recordOutput(VISION_ESTIMATED_POSE, estimatedPose);
    ...
    /**
     * The function returns the absolute field-oriented pose if the alliance is not red, otherwise
     * it returns the pose with the alliance flip applied.
     * 
     ...
     * @return The transformed trajectory (if the Alliance is red then then a deep copy will be
     *         transformed and made else the traj object will be returned)
     */
        ...
        states.forEach(s -> {
            if (getAlliance() == Alliance.Red) {
                adjusted.add(new State(s.timeSeconds, s.velocityMetersPerSecond,
                        ...
                        getAbsoluteFieldOrientedPoseFromAllianceOriented(s.poseMeters),
    ...
    /**
     * The function returns a list of target (AprilTag) translations sorted based on their distance
     * from the current robot position. (Ascending)
     * 
