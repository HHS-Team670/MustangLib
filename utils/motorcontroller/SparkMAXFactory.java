...
public class SparkMAXFactory {
    ...
    public static class Config {
        ...
        public int STATUS_FRAME_0_RATE_MS = 10; // Default status frame periods from
    ...
    public static final Config defaultConfig = new Config(); // For motors where we care about
                                                             // position and velocity
    public static final Config defaultVelocityConfig = new Config();// For motors were we only care
                                                                    // about precise velocity and
                                                                    // not precise position
    public static final Config defaultPositionConfig = new Config(); // For motors were we only care
                                                                     // about precise position and
                                                                     // not precise velocity
    public static final Config defaultLowUpdateRateConfig = new Config(); // For motors where we
                                                                          // neither care about
                                                                          // precise velocity or
                                                                          // position
    public static final Config defaultFollowerConfig = new Config(); // For follower motors

    // We leave frames 0 and 1 at default for non follower motors because we often need to track
    // motor applied output (Frame 0) and current (Frame 1) regadless of if we need precise position
    // or velocity
    static {
     ...
     * @param config The configuration to set this for, ex. default or defaultFollower
    ...
    /**
     * Used to build a pair of spark max controllers to control motors. Creates a leader on the port
     * which is working and makes other controller follow it
     * 
     * @param motor1DeviceID The CAN ID of spark max controller 1
     * @param motor2DeviceID The CAN ID of spark max controller 2
     * @return motorPair a pair of motors with the first one as its leader and second one as the
     *         follower
     */
    public static List<SparkMAXLite> buildFactorySparkMAXPair(int motor1DeviceID,
            int motor2DeviceID, boolean invertFollower, MotorConfig.Motor_Type motorType) {
        return buildSparkMAXPair(motor1DeviceID, motor2DeviceID, invertFollower, defaultConfig,
                defaultFollowerConfig, motorType);
    }
    ...
    public static List<SparkMAXLite> buildSparkMAXPair(int motor1DeviceID, int motor2DeviceID,
            boolean invertFollower, Config config, MotorConfig.Motor_Type motorType) {
        return buildSparkMAXPair(motor1DeviceID, motor2DeviceID, invertFollower, config, config,
                motorType);
    }
     ...
     */
    public static List<SparkMAXLite> buildSparkMAXPair(int motor1DeviceID, int motor2DeviceID,
            boolean invertFollower, Config leaderConfig, Config followerConfig,
            MotorConfig.Motor_Type motorType) {
        ...
        boolean isMotor1Error =
                sparkMaxLeaderError != REVLibError.kOk && sparkMaxLeaderError != null;
        boolean isMotor2Error =
                sparkMaxFollowerError != REVLibError.kOk && sparkMaxFollowerError != null;

        if (isMotor1Error && isMotor2Error) {
            MustangNotifications.reportError(
                    "SparkMaxControllerID %s and SparkMaxControllerID %s are broken",
                    sparkMaxLeader.getDeviceId(), sparkMaxFollower.getDeviceId());
        } else if (isMotor2Error) {
            MustangNotifications.reportWarning("SparkMaxControllerID %s is broken.",
                    sparkMaxFollower.getDeviceId());
        } else if (isMotor1Error) {
            MustangNotifications.reportWarning(
                    "SparkMaxControllerID %s is broken. Switching to SparkMaxControllerID %s",
                    sparkMaxLeader.getDeviceId(), sparkMaxFollower.getDeviceId());
        ...
        ConsoleLogger.consoleLog("SparkMaxLeaderID %s, SparkMaxFollowerID %s",
                sparkMaxLeader.getDeviceId(), sparkMaxFollower.getDeviceId());
        return motorPair;
    }
