 ...
 * {@link edu.wpi.first.math.trajectory.TrapezoidProfile} instead.
 */
public class SlewRateLimiter {
    ...
    public double calculate(double input) {
        ...
        if (input * m_prevVal > 0) { // accelerating
            m_prevVal += MathUtil.clamp(input - m_prevVal, -m_accelRateLimit * elapsedTime,
                    m_accelRateLimit * elapsedTime);
        } else { // decellerating
            m_prevVal += MathUtil.clamp(input - m_prevVal, -m_decelRateLimit * elapsedTime,
                    m_decelRateLimit * elapsedTime);
        }
