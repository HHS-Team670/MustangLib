...
public class Obstacle {
  ...
  public Obstacle offset(double distance) {
    ...
    for (int i = 0; i < polygon.npoints; i++) {
      offsetEdges.add(new ObstacleEdge(polygon.xpoints[i], polygon.ypoints[i],
          polygon.xpoints[(i + 1) % polygon.npoints], polygon.ypoints[(i + 1) % polygon.npoints])
              .offset(distance));
    }
  ...
  public class PolygonDouble implements Shape, Cloneable, Serializable {
    ...
    /**
     * The total number of points. The value of <code>npoints</code> represents the number of valid
     * points in this <code>Polygon</code>.
     */
    public int npoints;
     ...
     * @return <code>true</code> if the interior of this <code>Polygon</code> and the interior of
     *         the specified set of rectangular coordinates intersect each other; <code>false</code>
     *         otherwise.
     */
     ...
     * @return <code>true</code> if this <code>Polygon</code> entirely contains the specified set of
     *         rectangular coordinates; <code>false</code> otherwise.
     */
     ...
     * provides access to the geometry of the outline of this <code>Polygon</code>. An optional
     * {@link AffineTransform} can be specified so that the coordinates returned in the iteration
    ...
    /**
     * Returns an iterator object that iterates along the boundary of the <code>Polygon2D</code> and
     * provides access to the geometry of the outline of the <code>Shape</code>. Only SEG_MOVETO,
     * SEG_LINETO, and SEG_CLOSE point types are returned by the iterator. Since polygons are
     ...
     *        colinear before a subdivided curve is replaced by a straight line connecting the
     *        endpoints. Since polygons are already flat the <code>flatness</code> parameter is
     *        ignored.
  ...
  class Polyline2D implements Shape, Cloneable, Serializable {
    ...
    /**
     * The total number of points. The value of <code>npoints</code> represents the number of points
     * in this <code>Polyline2D</code>.
     */
    public int npoints;
    ...
    /**
     * Resets this <code>Polyline2D</code> object to an empty polygon. The coordinate arrays and the
     * data in them are left untouched but the number of points is reset to zero to mark the old
     * vertex data as invalid and to start accumulating new vertex data at the beginning. All
     * internally-cached data relating to the old vertices are discarded. Note that since the
     * coordinate arrays from before the reset are reused, creating a new empty
     * <code>Polyline2D</code> might be more memory efficient than resetting the current one if the
     * number of vertices in the new polyline data is significantly smaller than the number of
     ...
     * <p>
     * If an operation that calculates the bounding box of this <code>Polyline2D</code> has already
     * been performed, such as <code>getBounds</code> or <code>contains</code>, then this method
     ...
     * {@link Rectangle} whose sides are parallel to the x and y axes of the coordinate space, and
     * can completely contain the <code>Polyline2D</code>.
     *
    ...
    /**
     * Determines whether the specified {@link Point} is inside this <code>Polyline2D</code>. This
    ...
    /**
     * Determines if the specified coordinates are inside this <code>Polyline2D</code>. This method
     * is required to implement the Shape interface, but in the case of Line2D objects it always
    ...
    /**
     * Determines whether the specified coordinates are inside this <code>Polyline2D</code>. This
     * method is required to implement the Shape interface, but in the case of Line2D objects it
     * always returns false since a line contains no area.
     */
    public boolean contains(int x, int y) {
    ...
    /**
     * Tests if the interior of this <code>Polyline2D</code> entirely contains the specified set of
     * rectangular coordinates. This method is required to implement the Shape interface, but in the
     ...
     * <code>Rectangle2D</code>. This method is required to implement the Shape interface, but in
    ...
    /*
     * get the associated {@link Polygon2D}. This method take care that may be the last point can be
     * equal to the first. In that case it must not be included in the Polygon, as polygons declare
     * their first point only once.
     */
     ...
     * provides access to the geometry of the outline of the <code>Shape</code>. Only SEG_MOVETO and
     * SEG_LINETO, point types are returned by the iterator. Since polylines are already flat, the
