...
public abstract class MustangSubsystemBase extends SubsystemBase {
  ...
  /**
   * Creates a new MustangSubsystemBase. By default, the subsystem's initial health state is UNKNOWN
   * (ID 0).
   */
  ...
  /**
   * Checks the health of this subsystem and attempts to run this subsystem's mustangperiodic if the
   * health is yellow unknown or green
   */
  @Override
  public final void periodic() {
    ...
    if (lastHealth == HealthState.GREEN || lastHealth == HealthState.UNKNOWN
      ...
      if (failedLastTime) {
        MustangNotifications.notify(
            "Health state for " + this.getName() + " is: " + lastHealth + ". Enabling Periodic");
