...
public abstract class SparkMaxRotatingSubsystem extends MustangSubsystemBase
  ...
  public record Config(int kDeviceID, int kSlot, MotorConfig.Motor_Type kMotorType,
      IdleMode kIdleMode, double kRotatorGearRatio, double kP, double kI, double kD, double kFF,
      double kIz, double kMaxOutput, double kMinOutput, double kMaxRotatorRPM,
   ...
   * gear ratio.
   *
   * @param rotRPM The parameter "rotRPM" represents the rotational speed of a subsystem, rotations
   *        per minute
  ...
  /**
   * The function checks if a given setpoint is within the soft limits defined in the kConfig
   * object.
   *
  ...
  private boolean checkSoftLimits(double setpoint) {
    ...
    if (kConfig.kSoftLimits != null
        && (setpoint > kConfig.kSoftLimits[0] || setpoint < kConfig.kSoftLimits[1])) {
      ConsoleLogger.consoleLog(
          "In " + getName() + " Improper setpoint: " + setpoint + " Setpoint should be between "
              + kConfig.kSoftLimits[1] + " and " + kConfig.kSoftLimits[0]);
      return false;
    }
    return true;
  }
  ...
  /**
   * The function sets the motion target for a system, but first checks if the setpoint is within
   * the soft limits and logs an error message if it is not.
   *
   ...
   *        for any external forces or disturbances acting on the system. It is an arbitrary value
   *        that you can adjust to achieve the desired response of the system.
   * @return if this action was sucessful
   */
  protected boolean setSystemMotionTarget(double setpoint, double arbitraryFF) {
    if (checkSoftLimits(setpoint)) {
      if (setpoint != kNoSetPoint) {
        mController.setReference(setpoint, CANSparkMax.ControlType.kSmartMotion, 0, arbitraryFF);
  ...
  /**
   * Sets an intermediate or temporary goal for the subsystem to move to. Use this when you need to
   * do something in the process of moving to your system's target, like if you need to unjam
   * something.
   *
  ...
  /**
   * Sets the overall target angle this subsystem should move to, in degrees. In some process, this
   * is where you ultimately want to end up, so this value will be saved as the system setpoint.
   *
  ...
  /**
   * Sets a temporary angle setpoint for this subsystem to turn to. Use this as an intermediate step
   * in some process -- this setpoint won't be saved.
   *
   ...
   * @return The number of motor rotations, as measured by the encoder, equivalent to the subsystem
   *         turning through this angle
   */
  protected double getMotorRotationsFromAngle(double angle) {
    double rotations = (angle / 360) * kConfig.kRotatorGearRatio
        + ((int) (getUnadjustedPosition() / kConfig.kRotatorGearRatio)) * kConfig.kRotatorGearRatio;
    // Logger.consoleLog("Indexer motor rotations from angle is %s", rotations);
    return rotations;
  }

  /**
   * Change the system's feedforward and max velocity and acceleration temporarily. Possibly useful
   * when zeroing, testing, or unjamming.
   *
