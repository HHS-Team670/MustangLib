...
public abstract class SwerveDrive extends DriveBase {
  ...
  public static record Config(double kDriveBaseTrackWidth, double kDriveBaseWheelBase,
      ...
      int kBackLeftModuleDriveMotor, int kBackLeftModuleSteerMotor, int kBackLeftModuleSteerEncoder,
      double kBackLeftModuleSteerOffset, AbsoluteEncoderType kBackLeftModuleEncoderType,
  ...
  public SwerveDrive(Config config) {
    ...
    kKinematics = new SwerveDriveKinematics(
        // Front left
        new Translation2d(config.kDriveBaseTrackWidth / 2.0, config.kDriveBaseWheelBase / 2.0),
        // Front right
        new Translation2d(config.kDriveBaseTrackWidth / 2.0, -config.kDriveBaseWheelBase / 2.0),
        // Back left
        new Translation2d(-config.kDriveBaseTrackWidth / 2.0, config.kDriveBaseWheelBase / 2.0),
    ...
    kKinematics.resetHeadings(new Rotation2d(config.headingOffsetRadians),
        new Rotation2d(config.headingOffsetRadians), new Rotation2d(config.headingOffsetRadians),
  ...
  public void drive(double xVel, double yVel, double thetaVel) {
    ...
    setModuleStates(kKinematics.toSwerveModuleStates(
        ChassisSpeeds.fromFieldRelativeSpeeds(xVel, yVel, thetaVel, this.getGyroscopeRotation())));
  }
  ...
  public void mustangPeriodic() {
    ...
    if (kConfig.kDriveMotorType.equals(Motor_Type.KRAKEN_X60)) {
      ...
      for (int i = 0; i < mModules.length; i++) {
        latestDriveCurrentDraw +=
            ((TalonFX) mModules[i].getDriveMotor()).getSupplyCurrent().getValueAsDouble();
      }
      if (!bufferFull) {
  ...
  public void resetOdometry(Pose2d pose) {
    ...
    setGyroscopeRotation(Rotation2d
        .fromDegrees(getGyroscopeRotation(false).getDegrees() + pose.getRotation().getDegrees()));
  ...
  public void configureHolonomic() {
    ...
    HolonomicPathFollowerConfig config =
        new HolonomicPathFollowerConfig(RobotConstantsBase.SwerveDriveBase.kAutonTranslationPID,
  ...
  private class RotationController {
    ...
    public double calculateRotationSpeed(Rotation2d currentHeading, Rotation2d desiredHeading) {
      double thetaFF =
          m_thetaController.calculate(currentHeading.getRadians(), desiredHeading.getRadians());
