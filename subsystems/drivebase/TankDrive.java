...
public abstract class TankDrive extends DriveBase {
  ...
  private final String HEADING = getName() + "/Heading";
  private final String CURRENTX = getName() + "/currentX";
  private final String CURRENTY = getName() + "/currentY";
  private final String LEFTENCODER = getName() + "/left1Encoder";
  private final String RIGHTENCODER = getName() + "/right1Encoder";
  private final String LEFTVELOCITY = getName() + "/leftVelocity";
  private final String RIGHTVELOCITY = getName() + "/rightVelocity";
  private final String LEFTPOS = getName() + "/leftPos";
  private final String RIGHTPOS = getName() + "/rightPos";
  ...
  public static record Config(double kDriveBaseTrackWidth, double kDriveBaseGearRatio, double kDriveBaseWheelDiameter,
      ...
      int kRightFollowerMotorID, double deadband, boolean inverted, SerialPort.Port kNavPort, boolean kNavXReversed,
      AutonConfig autonConfig) {
  }

  // Added an autonconfig to break up the config into easy to read chuncks.
  // additionally, all of these values need to be updated whenever changes are
  // amde to the tankdrive
  public static record AutonConfig(int kTimeoutMs, double leftKsVolts, double leftKvVoltSecondsPerMeter,
      ...
      double rightKaVoltSecondsSquaredPerMeter, PIDController kLeftController, PIDController kRightController,
   ...
   * system.
   * 
   * @param leftVoltage  The voltage to be applied to the left side of the tank
   *                     drive system. This will
   *                     control the speed and direction of the left side of the
   *                     tank drive.
   * @param rightVoltage The rightVoltage parameter is the desired voltage to be
  ...
  /**
   * @return The method is returning the left velocity of the tank drive base in
   *         ticks per second.
   */
  ...
  /**
   * @return The method is returning the right velocity of the tank drive base in
   *         ticks per second.
   */
  ...
  public void debugSubsystem() {
    ...
    Logger.recordOutput(LEFTENCODER, getLeftPositionTicks());
    Logger.recordOutput(RIGHTENCODER, getRightPositionTicks());

    Logger.recordOutput(LEFTVELOCITY, left1Encoder.getVelocity());
    Logger.recordOutput(RIGHTVELOCITY, right1Encoder.getVelocity());

    Logger.recordOutput(LEFTPOS, left1Encoder.getPosition());
    Logger.recordOutput(RIGHTPOS, right1Encoder.getPosition());
