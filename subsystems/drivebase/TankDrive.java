...
public abstract class TankDrive extends DriveBase {
  ...
  public static record Config(double kDriveBaseTrackWidth, double kDriveBaseGearRatio,
      double kDriveBaseWheelDiameter, double kTrackWidthMeters, int kLeftLeaderMotorID,
      int kLeftFollowerMotorID, int kRightLeaderMotorID, int kRightFollowerMotorID, double deadband,
      boolean inverted, SerialPort.Port kNavPort, boolean kNavXReversed, AutonConfig autonConfig) {
  }
  // Added an autonconfig to break up the config into easy to read chuncks. additionally, all of
  // these values need to be updated whenever changes are amde to the tankdrive
  public static record AutonConfig(int kTimeoutMs, double leftKsVolts,
      double leftKvVoltSecondsPerMeter, double leftKaVoltSecondsSquaredPerMeter,
      double rightKsVolts, double rightKvVoltSecondsPerMeter,
      double rightKaVoltSecondsSquaredPerMeter, PIDController kLeftController,
      PIDController kRightController, double kMaxSpeedMetersPerSecond,
  ...
  public TankDrive(Config kConfig) {
     ...
     */
    leftControllers = SparkMAXFactory.buildFactorySparkMAXPair(kConfig.kLeftLeaderMotorID,
        kConfig.kLeftFollowerMotorID, false, MotorConfig.Motor_Type.NEO);
    /**
    ...
    for (SparkMAXLite motorController : allMotors) {
      ...
      motorController.getEncoder()
          .setPositionConversionFactor(getDrivebaseVelocityConversionFactor());
    }
    ...
    setMotorsInvert(rightControllers, true); // Invert right controllers here so they will work
                                             // properly with the CANPIDController
   ...
   * @param rightSideInverted Invert the right motor outputs to counteract them being flipped
   *        comparatively with the left ones
   * @param deadband A minimum motor input to move the drivebase
   * @param safetyEnabled Safety Mode, enforces motor safety which turns off the motors if
   *        communication lost, other failures, etc.
   */
  protected void setMotorControllers(MotorController leftMotor, MotorController rightMotor,
      boolean inverted, boolean rightSideInverted, double deadband, boolean safetyEnabled) {
   ...
   *        comparatively with the left ones
   */
  public void setMotorControllers(MotorController leftMotor, MotorController rightMotor,
      boolean inverted, boolean rightSideInverted) {
    setMotorControllers(leftMotor, rightMotor, inverted, rightSideInverted, 0.02, true);
  }

  /**
   * Checks the health for driveBase. RED if all motors are dead, GREEN if all motors are alive,
   * YELLOW if a motor is disconnected.
   *
  ...
  public DifferentialDriveWheelSpeeds getWheelSpeeds() {
    return new DifferentialDriveWheelSpeeds(left1Encoder.getVelocity(),
        right1Encoder.getVelocity());
  }
  ...
  /**
   * Checks the health for driveBase. RED if all motors are dead, GREEN if all motors are alive and
   * navx is connected, YELLOW if a motor is disconnected or navX is not connected
   */

  public HealthState checkHealth() {
    HealthState motorHealth =
        checkHealth(left1.isErrored(), left2.isErrored(), right1.isErrored(), right2.isErrored());
  ...
  public void mustangPeriodic() {
    odometry.update(Rotation2d.fromDegrees(getHeading()), left1Encoder.getPosition(),
        right1Encoder.getPosition());
    if (SmartDashboard.getBoolean("aligned", false)
        && !MathUtils.doublesEqual(getHeading(), prevHeading, 0)) {
   ...
   *        right side of the tank drive system.
   */
  ...
  public double getLeftPositionTicks() {
    return (int) (left1Encoder.getPosition()
        / RobotConstantsBase.TankDriveBase.kSparkTicksPerRotation);
  }
  ...
  public double getLeftVelocityTicks() {
    return (left1Encoder.getVelocity() / RobotConstantsBase.TankDriveBase.kSparkTicksPerRotation
        / 60);
  }
  ...
  public double getRightVelocityTicks() {
    return (right1Encoder.getVelocity() / RobotConstantsBase.TankDriveBase.kSparkTicksPerRotation
        / 60);
  }
  ...
  /**
   * Returns the velocity of the right side of the drivebase in inches/second from the Spark Encoder
   */
  ...
  public SimpleMotorFeedforward getLeftSimpleMotorFeedforward() {
    return new SimpleMotorFeedforward(kConfig.autonConfig.leftKsVolts,
        kConfig.autonConfig.leftKvVoltSecondsPerMeter,
  ...
  public SimpleMotorFeedforward getRightSimpleMotorFeedforward() {
    return new SimpleMotorFeedforward(kConfig.autonConfig.rightKsVolts,
        kConfig.autonConfig.rightKvVoltSecondsPerMeter,
  ...
  public void holdPosition() {
    getLeftSparkPIDController().setReference(left1Encoder.getPosition(),
        CANSparkMax.ControlType.kPosition);
    getRightSparkPIDController().setReference(right1Encoder.getPosition(),
        CANSparkMax.ControlType.kPosition);
  }
