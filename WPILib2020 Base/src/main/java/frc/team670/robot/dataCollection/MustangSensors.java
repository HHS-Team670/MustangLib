...
/**
 * Instantiates sensor representation objects and contains methods for accessing
 * the sensor data.
 * 
 * @author shaylandias
 */
public class MustangSensors {
  ...
  private static final double FRONT_ULTRA_OFFSET = -12.25, BACK_LEFT_ULTRA_OFFSET = -9.25,
      BACK_RIGHT_ULTRA_OFFSET = 9.25;
  ...
  public MustangSensors() {
    ...
    try {
      ...
      // DriverStation.reportError("Error instantiating intakeIRSensor: " +
      // ex.getMessage(), true);
    ...
    try {
      frontUltrasonic = new DIOUltrasonic(RobotMap.FRONT_ULTRASONIC_TRIGGER_PIN, RobotMap.FRONT_ULTRASONIC_ECHO_PIN,
          FRONT_ULTRA_OFFSET);
    } catch (RuntimeException ex) {
      // DriverStation.reportError("Error instantiating front ultrasonic: " +
      // ex.getMessage(), true);
    ...
    try {
      backLeftUltrasonic = new DIOUltrasonic(RobotMap.BACK_LEFT_ULTRASONIC_TRIGGER_PIN,
          RobotMap.BACK_LEFT_ULTRASONIC_ECHO_PIN, BACK_LEFT_ULTRA_OFFSET);
    } catch (RuntimeException ex) {
      // DriverStation.reportError("Error instantiating back left ultrasonic: " +
      // ex.getMessage(), true);
      // SmartDashboard.putString("warning", "Error instantiating back left
      // ultrasonic");
      backLeftUltrasonic = null;
    }

    try {
      backRightUltrasonic = new DIOUltrasonic(RobotMap.BACK_RIGHT_ULTRASONIC_TRIGGER_PIN,
          RobotMap.BACK_RIGHT_ULTRASONIC_ECHO_PIN, BACK_RIGHT_ULTRA_OFFSET);
    } catch (RuntimeException ex) {
      // DriverStation.reportError("Error instantiating back right ultrasonic: " +
      // ex.getMessage(), true);
      // SmartDashboard.putString("warning", "Error instantiating back right
      // ultrasonic");
      backRightUltrasonic = null;
    }

    frontUltrasonic.setUltrasonicAutomaticMode(true); // This will set it for all the sensors. WPI should definitely
                                                      // have made this method static.
  }
   ...
   * If both ultrasonics have similar data it averages; if only one has "good"
   * data it uses the adjusted distance
   */
  public double getAdjustedBackUltrasonicDistance() {
    ...
    // Below check if both sensors are getting valid data, if not pick the one that
    // seems right, or return the error code
  ...
  /**
   * Gets the Rotation for the Pure Pursuit drive. (-180, 180) with 90 being
   * forward
   */
  ...
  /**
   * Resets the NavX. This means that the NavX performs a shut-down and
   * recalibration, taking time. Use this if it significantly drifs.
   */
  ...
  /**
   * Returns the rate of change of the yaw angle in degrees per second. If NavX
   * not connected, returns NAVX_ERROR_CODE.
   */
  ...
  /**
   * Returns the yaw of the robot as a double in accordance with its adjustment
   * based on Robot starting position. (-180, 180)
   * 
   * @return The yaw of the robot if the navX is connected. Otherwise returns
   *         NAVX_ERROR_CODE
   */
   ...
   * (180, -180). If NavX not connected, returns NAVX_ERROR_CODE
   * 
   ...
   * Returns null if the navX is not connected.
   */
  ...
  /**
   * Adds a "user-offset" variable to the NavX, effectively zeroing the value you
   * will receive at this point.
   */
  public void zeroYaw() {
    if (navXMicro != null) {
      navXMicro.zeroYaw();
  ...
  /*
   * Gets the angle needed to turn to the nearest target - based on navx
   * fieldcentric angle and fieldcentric angle of the target to determine which
   * target you're looking at realistically - uses this for offset calculations on
   * camera/ultrasonic
   */
  ...
  /**
   * Returns a PIDSource with the NavX Yaw corresponding to the last zero (not
   * field centric).
   * 
  ...
  /**
   * @return true if NavX is null (could not be instantiated), false if you can
   *         call methods on the NavX
   */
  ...
  /**
   * Sends the Ultrasonic data from sensors to Dashboard
   */
  ...
  /**
   * Sends the BreamBake data from sensor to dashboard
   */
